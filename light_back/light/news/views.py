...
def new_content(request, task_id):
    try:

        task = get_object_or_404(Task, pk=task_id)
        result = task.new_result
        ...
        if request.method == 'GET':
            serializer = NewResultSerializer(result, context={'request': request})
...
@api_view(['PATCH'])
@permission_classes([AllowAny])
def new_review(request, task_id):
    try:
