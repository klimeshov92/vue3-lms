<template>
  <div v-if="loading">
    <div v-if="state.canViewChat" class="detail-page">
      <div v-if="state.object.id">
        <div class="detail-card"> 
          <div class="detail-card-info">
            ...
            <div class="detail-menu button-group">
              <router-link 
                v-if="state.canEditChat" 
                :to="{ name: 'ChatEdit', params: { id: state.object.id } }"
                class="button"
              >
                Изменить
              </router-link>
              <button 
                v-if="state.canDeleteChat" 
                @click="openChatDeleteModal"
                class="button"
              >
                Удалить
              </button>
            </div>
            <div v-if="showChatDeleteModal" class="modal-overlay">
              <div class="modal">
                <div class="modal-header">
                  <h2 class="modal-header-h2">Удаление {{ state.object.name || 'Безымянный чат' }}</h2>
                </div>
                <div class="minibutton-group modal-menu">
                  <button @click="confirmChatDelete()" class="minibutton">Подтвердить</button>
                  <button @click="closeChatDeleteModal" class="minibutton">Отменить</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="detail-tabs">
          ...
          <div v-if="activeTab" class="tabs-content">
            ...
            <div v-if="activeTab === 'messages'" class="chat-tab">
            ...
            <div v-if="activeTab === 'accountsGroupObjectPermissions'" class="table-tab">
              ...
              <AccountsGroupObjectPermissions :state="state" :fetchObject="fetchObject" :contentTypeModel="'chat'" />

            </div>

            <div v-if="activeTab === 'accountObjectPermissions'" class="table-tab">

              <AccountObjectPermissions :state="state" :fetchObject="fetchObject" :contentTypeModel="'chat'" />
...
const fetchObject = async () => {
  ...
  try {
    ...
    console.log('Данные чата:', response.data); 
    state.object = response.data;
    chat_id.value = state.object.id; // Обновляем реактивную переменную chat_id
    console.log('ID чата:', chat_id.value);
  } catch (error) {
    console.error('Ошибка при получении данных чата:', error);
  }
};

const showChatDeleteModal = ref(false);
const openChatDeleteModal = () => {
  showChatDeleteModal.value = true;
};
const closeChatDeleteModal = () => {
  showChatDeleteModal.value = false;
};
const confirmChatDelete = async () => {
  ...
  const id = route.params.id;
  try {
    await axios.delete(`${baseUrl}/chats/${id}/`, {
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddChat = state.globalPermissionsList.includes('chats.add_chat');
    console.log('Права на добавление чат:', state.canAddChat);
    ...
    state.canEditChat = state.globalPermissionsList.includes('chats.change_chat') ||
      (Array.isArray(state.objectPermissionsDict['chats.change_chat']) &&
      state.objectPermissionsDict['chats.change_chat'].includes(Number(id)));
    console.log('Права на редактирование чата:', state.canEditChat);

    state.canDeleteChat = state.globalPermissionsList.includes('chats.delete_chat') ||
      (Array.isArray(state.objectPermissionsDict['chats.delete_chat']) &&
      state.objectPermissionsDict['chats.delete_chat'].includes(Number(id)));
    console.log('Права на удаление чата:', state.canDeleteChat);
...
const activeTab = ref(route.query.tab || localStorage.getItem(`activeChatTab-${route.params.id}`) || 'desc');
watch(activeTab, (newTab) => {
  localStorage.setItem(`activeChatTab-${route.params.id}`, newTab);
