<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewInteraction" class="list-page">
        ...
        <div class="filters-outer" @click.stop>
          ...
          <div class="filters-inner">
            ...
            <div class="filters-field">
              <label class="filters-label">ID:</label>
              <input v-model="filters.id" class="filters-input" placeholder="Введите идентификатор" />
            </div>
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'InteractionDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddInteraction" class="list-menu button-group">

        <router-link
          v-if="state.canAddInteraction"
          :to="{ name: 'InteractionCreate' }"
          class="button"
        >
          Создать
        </router-link>
...
const orders = [
  { label: 'Идентификатор: по возрастанию', value: 'id' },
  { label: 'Идентификатор: по убыванию', value: '-id' },
];
...
const filters = reactive({
  ...
  categories: route.query.categories
    ...
    : [],
  order: orders.find(option => option.value === route.query.order || 'id')
});

const loadFilters = () => {
  const storedFilters = localStorage.getItem('interactionFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('interactionPage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  ...
  localStorage.setItem('interactionFilters', JSON.stringify(filters))
...
const changePage = (page) => {
  ...
  localStorage.setItem('interactionPage', currentPage.value);
...
const resetFilters = () => {
  ...
  filters.id = '';
  filters.categories = [];
  filters.order = orders.find(option => option.value === 'id');

  localStorage.removeItem('interactionFilters');
  localStorage.removeItem('interactionPage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canViewInteraction = state.globalPermissionsList.includes('bpms.view_interaction') || !!state.objectPermissionsDict['bpms.view_interaction'];
    console.log('Права на просмотр задач:', state.canViewInteraction);
