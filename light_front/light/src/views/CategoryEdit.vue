    ...
    <div v-if="object" class="form-page">
      ...
      <form @submit.prevent="saveChanges" class="form" id="edit-form">
        ...
        <div class="form-field">
          ...
          <span v-if="errors.parent_category" class="error">{{ errors.parent_category }}</span>
        </div>
...
const validateForm = () => {
  errors.name = form.name ? '' : 'Название названия обязательно!';
  if (form.parent_category && form.parent_category.id === object.value.id) {
    errors.parent_category = 'Родительская категория не может быть текущей категорией!';
  } else {
    delete errors.parent_category; 
  }
...
const saveChanges = async () => {
  ...
  try {
    ...
    const jsonData = {
      name: form.name,
      desc: form.desc,
      parent_category: form.parent_category?.id,
    };

    console.log('JSON данные перед отправкой:', jsonData);

    await axios.patch(`${baseUrl}/categories/${id}/`, jsonData, {
      headers: { Authorization: `Bearer ${token}` },
    });
