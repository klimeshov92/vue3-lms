<template>
  <div v-if="loading">
    <div v-if="state.canViewLegalPage" class="detail-page">
      <div v-if="state.object.id">
        <div class="detail-card"> 
          <div class="detail-card-info">
            ...
            <div v-if="state.canEditLegalPage" class="detail-menu button-group">

              <router-link
                v-if="state.canEditLegalPage" 
                :to="{ name: 'LegalPageEdit', params: { id: state.object.id } }"
                class="button"
              >
                Изменить
              </router-link>
      ...
      <div v-else-if="!state.object.id" class="none-container">
        ...
        <div v-if="state.canAddLegalPage" class="detail-menu button-group none-menu-padding">

          <router-link
            :to="{ name: 'LegalPageCreate' }"
            class="button"
          >
            Создать
          </router-link>
...
const state = reactive({
  ...
  canViewLegalPage: false,
  canEditLegalPage: false,
  canAddLegalPage: false,
});
...
const fetchObject = async () => {
  ...
  try {
    const response = await axios.get(`${baseUrl}/legal_page_content/`, {
      headers: {
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canViewLegalPage = true

    state.canAddLegalPage = state.globalPermissionsList.includes('core.add_legal_page');
    console.log('Права на добавление страницы:', state.canAddLegalPage);

    state.canEditLegalPage = state.globalPermissionsList.includes('core.change_legal_page') ||
      (Array.isArray(state.objectPermissionsDict['core.change_legal_page']) &&
      state.objectPermissionsDict['core.change_legal_page'].includes(Number(id)));
    console.log('Права на редактирование страницы:', state.canEditLegalPage);
