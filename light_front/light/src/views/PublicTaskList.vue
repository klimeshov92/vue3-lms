<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewPublicTask" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'PublicTaskDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddPublicTask" class="list-menu button-group">

        <router-link
          v-if="state.canAddPublicTask"
          :to="{ name: 'PublicTaskCreate' }"
          class="button"
        >
          Создать
        </router-link>
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('public_taskFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('public_taskPage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка планов с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  localStorage.setItem('public_taskFilters', JSON.stringify(filters))
  ...
  try {
    ...
    console.log('Список планов загружен:', response.data); 
...
const changePage = (page) => {
  ...
  localStorage.setItem('public_taskPage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('public_taskFilters');
  localStorage.removeItem('public_taskPage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddPublicTask = state.globalPermissionsList.includes('bpms.add_public_task');
    console.log('Права на добавление планов:', state.canAddPublicTask);
    
    state.canViewPublicTask = state.globalPermissionsList.includes('bpms.view_public_task') || !!state.objectPermissionsDict['bpms.view_public_task'];
    console.log('Права на просмотр планов:', state.canViewPublicTask);
