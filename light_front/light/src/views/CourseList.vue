<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewCourse" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'CourseDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddCourse" class="list-menu button-group">

        <router-link
          v-if="state.canAddCourse"
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('courseFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('coursePage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка курсов с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  try {
    ...
    console.log('Список курсов загружен:', response.data); 
...
const changePage = (page) => {
  ...
  localStorage.setItem('coursePage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('coursePage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddCourse = state.globalPermissionsList.includes('courses.add_course');
    console.log('Права на добавление курсов:', state.canAddCourse);
    
    state.canViewCourse = state.globalPermissionsList.includes('courses.view_course') || !!state.objectPermissionsDict['courses.view_course'];
