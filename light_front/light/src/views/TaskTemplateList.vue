<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewTaskTemplate" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'TaskTemplateDetail', params: { id: item.id },  query: { tab: 'desc' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddTaskTemplate" class="list-menu button-group">

        <router-link
          v-if="state.canAddTaskTemplate"
          :to="{ name: 'TaskTemplateCreate' }"
          class="button"
        >
          Создать
        </router-link>
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('taskTemplateFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('taskTemplatePage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка шаблонов задач с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  localStorage.setItem('taskTemplateFilters', JSON.stringify(filters))
  ...
  try {
    ...
    console.log('Список шаблонов задач загружен:', response.data); 
    state.items = response.data.results;
    //state.items = [];
    totalPages.value = Math.ceil(response.data.count / 12);
  } catch (error) {
    console.error('Ошибка при загрузке списка шаблонов задач:', error);
...
const changePage = (page) => {
  ...
  localStorage.setItem('taskTemplatePage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('taskTemplateFilters');
  localStorage.removeItem('taskTemplatePage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canViewTaskTemplate = state.globalPermissionsList.includes('bpms.view_task_template') || !!state.objectPermissionsDict['bpms.view_task_template'];
    console.log('Права на просмотр шаблонов задач:', state.canViewTaskTemplate);
