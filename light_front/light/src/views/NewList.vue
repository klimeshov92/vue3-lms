<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewNew" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'NewDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddNew" class="list-menu button-group">

        <router-link
          v-if="state.canAddNew"
          :to="{ name: 'NewCreate' }"
          class="button"
        >
          Создать
...
const orders = [
  { label: 'Дата: по возрастанию', value: 'created' },
  { label: 'Дата: по убыванию', value: '-created' },
...
const filters = reactive({
  ...
  categories: route.query.categories
    ...
    : [],
  order: orders.find(option => option.value === route.query.order || 'created')
});

const loadFilters = () => {
  const storedFilters = localStorage.getItem('newFilters');
  if (storedFilters) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка новостей с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  localStorage.setItem('newFilters', JSON.stringify(filters))
  ...
  try {
    ...
    console.log('Список новостей загружен:', response.data); 
...
const changePage = (page) => {
  ...
  localStorage.setItem('newPage', currentPage.value);
...
const resetFilters = () => {
  ...
  filters.name = '';
  filters.categories = [];
  filters.order = orders.find(option => option.value === 'created');

  localStorage.removeItem('newFilters');
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddNew = state.globalPermissionsList.includes('news.add_new');
    console.log('Права на добавление новостей:', state.canAddNew);
    
    state.canViewNew = state.globalPermissionsList.includes('news.view_new') || !!state.objectPermissionsDict['news.view_new'];
    console.log('Права на просмотр новостей:', state.canViewNew);
