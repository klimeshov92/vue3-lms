<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewMaterial" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'MaterialDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddMaterial" class="list-menu button-group">

        <router-link
          v-if="state.canAddMaterial"
          :to="{ name: 'MaterialCreate' }"
          class="button"
        >
          Создать
        </router-link>
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('materialFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('materialPage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка материалов с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  try {
    ...
    console.log('Список материалов загружен:', response.data); 
    state.items = response.data.results;
    //state.items = [];
    totalPages.value = Math.ceil(response.data.count / 12);
  } catch (error) {
    console.error('Ошибка при загрузке списка материалов:', error);
...
const changePage = (page) => {
  ...
  localStorage.setItem('materialPage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('materialPage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddMaterial = state.globalPermissionsList.includes('materials.add_material');
    console.log('Права на добавление материалов:', state.canAddMaterial);
    
    state.canViewMaterial = state.globalPermissionsList.includes('materials.view_material') || !!state.objectPermissionsDict['materials.view_material'];
    console.log('Права на просмотр материалов:', state.canViewMaterial);
