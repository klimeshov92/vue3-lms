<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewEventTemplate" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'EventTemplateDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddEventTemplate" class="list-menu button-group">

        <router-link
          v-if="state.canAddEventTemplate"
          :to="{ name: 'EventTemplateCreate' }"
          class="button"
        >
          Создать
        </router-link>
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('eventTemplateFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('eventTemplatePage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  ...
  localStorage.setItem('eventTemplateFilters', JSON.stringify(filters))
...
const changePage = (page) => {
  ...
  localStorage.setItem('eventTemplatePage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('eventTemplateFilters');
  localStorage.removeItem('eventTemplatePage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddEventTemplate = state.globalPermissionsList.includes('events.add_event_template');
    console.log('Права на добавление шаблонов:', state.canAddEventTemplate);
    
    state.canViewEventTemplate = state.globalPermissionsList.includes('events.view_event_template') || !!state.objectPermissionsDict['events.view_event_template'];
    console.log('Права на просмотр шаблонов:', state.canViewEventTemplate);
