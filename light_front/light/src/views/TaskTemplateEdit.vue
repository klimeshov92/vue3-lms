  ...
  <div v-if="loading">
    ...
    <div v-if="object" class="form-page">
      ...
      <div class="form-header">
        <h1>Редактирование шаблона задачи</h1>
      </div>
      ...
      <form @submit.prevent="saveChanges" class="form" id="edit-form">
        ...
        <div v-if="form.term_type && form.term_type.value != 'none'" class="form-field">
          ...
          <input v-model="form.term_value" id="term_value" type="number" class="form-input" placeholder="Введите срок" />
...
const fetchObject = async () => {
  ...
  try {
    ...
    object.value.term_type = term_types.find(option => option.value === object.value.term_type);
...
const saveChanges = async () => {
  ...
  try {
    ...
    await axios.patch(`${baseUrl}/task_templates/${id}/`, jsonData, {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    if (planId){
      ...
      router.push({ name: 'TaskTemplateDetail', params: { id: id } });
    }
