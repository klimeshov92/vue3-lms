<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewQuestion" class="list-page">
        ...
        <div class="filters-outer" @click.stop>
          ...
          <div class="filters-inner">
            ...
            <div class="filters-field">
              ...
              <input v-model="filters.text" class="filters-input" placeholder="Введите название" />
            </div>
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-image-outer">
              <div v-if="item.picture" class="list-card-image-inner">
                <img :src="item.picture" alt="Картинка" />
              </div>
              <div v-else class="list-card-image-none">
                <span class="none-text">Нет картинки</span>
              </div>
            </div>

            <div class="list-card-header">
              <div class="list-card-header-title">
                <h2>{{ item.text || 'Нет текста' }}</h2>
              </div>
            </div>
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'QuestionDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddQuestion" class="list-menu button-group">

        <router-link
          v-if="state.canAddQuestion"
...
const orders = [
  { label: 'Текст: по возрастанию', value: 'text' },
  { label: 'Текст: по убыванию', value: '-text' },
];
...
const filters = reactive({
  ...
  categories: route.query.categories
    ...
    : [],
  order: orders.find(option => option.value === route.query.order || 'text')
});

const loadFilters = () => {
  const storedFilters = localStorage.getItem('questionFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('questionPage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка вопросов с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  try {
    ...
    console.log('Список вопросов загружен:', response.data); 
...
const changePage = (page) => {
  ...
  localStorage.setItem('questionPage', currentPage.value);
...
const resetFilters = () => {
  ...
  filters.text = '';
  filters.categories = [];
  filters.order = orders.find(option => option.value === 'text');
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddQuestion = state.globalPermissionsList.includes('tests.add_question');
    console.log('Права на добавление вопросов:', state.canAddQuestion);
    
    state.canViewQuestion = state.globalPermissionsList.includes('tests.view_question') || !!state.objectPermissionsDict['tests.view_question'];
