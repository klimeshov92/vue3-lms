<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewChat" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'ChatDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddChat" class="list-menu button-group">

        <router-link
          v-if="state.canAddChat"
          :to="{ name: 'ChatCreate' }"
          class="button"
        >
          Создать
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('chatFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('chatPage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка чатов с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  try {
    ...
    console.log('Список чатов загружен:', response.data); 
...
const changePage = (page) => {
  ...
  localStorage.setItem('chatPage', currentPage.value);
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    console.log('Права на добавление чатов:', state.canAddChat);
    
    state.canViewChat = state.globalPermissionsList.includes('chats.view_chat') || !!state.objectPermissionsDict['chats.view_chat'];
    console.log('Права на просмотр чатов:', state.canViewChat);
