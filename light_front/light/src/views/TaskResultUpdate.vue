  ...
  <div v-if="loading">
    ...
    <div v-if="object" class="form-page">
      ...
      <div class="form-header">
        <h1>Результат задачи</h1>
      </div>

      <form @submit.prevent="saveChanges" class="modal-form" id="edit-form">

        <div class="form-field">
          <label for="score_scaled" class="form-label">Процент выполнения:</label>
          <input v-model="form.score_scaled" id="score_scaled" type="number" class="form-input" placeholder="Введите пункт" />
          <span v-if="errors.score_scaled" class="error">{{ errors.score_scaled }}</span>
        </div>
...
const fetchObject = async () => {
  ...
  try {
    const response = await axios.get(`${baseUrl}/task_result_update/${id}/`, {
...
const validateForm = () => {
  ...
  errors.score_scaled = form.score_scaled != null ? '' : 'Выбор процента выполнения обязателен!';
...
const saveChanges = async () => {
  ...
  try {
    ...
    const jsonData = {
      score_scaled: form.score_scaled,
    ...
    await axios.patch(`${baseUrl}/task_result_update/${id}/`, jsonData, {
      headers: { Authorization: `Bearer ${token}` },
    });
