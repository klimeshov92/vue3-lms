<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewControlElement" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'ControlElementDetail', params: { id: item.id },  query: { tab: 'desc' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddControlElement" class="list-menu button-group">

        <router-link
          v-if="state.canAddControlElement"
          :to="{ name: 'ControlElementCreate' }"
          class="button"
        >
          Создать
        </router-link>
...
const filters = reactive({
  ...
  categories: route.query.categories
    ...
    : [],
  order: orders.find(option => option.value === route.query.order || 'planned_end')
});

const loadFilters = () => {
  const storedFilters = localStorage.getItem('control_elementFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('control_elementPage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка управляющих элементов с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  localStorage.setItem('control_elementFilters', JSON.stringify(filters))
...
const changePage = (page) => {
  ...
  localStorage.setItem('control_elementPage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('control_elementFilters');
  localStorage.removeItem('control_elementPage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddControlElement = state.globalPermissionsList.includes('bpms.add_control_element');
    console.log('Права на добавление задач:', state.canAddControlElement);
    
    state.canViewControlElement = state.globalPermissionsList.includes('bpms.view_control_element') || !!state.objectPermissionsDict['bpms.view_control_element'];
