<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewClient" class="list-page">
        ...
        <div class="filters-outer" @click.stop>
          ...
          <div class="filters-inner">
            ...
            <div class="filters-field">
              ...
              <input v-model="filters.name" class="filters-input" placeholder="Введите идентификатор" />
            </div>
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'ClientDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddClient" class="list-menu button-group">

        <router-link
          v-if="state.canAddClient"
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('clientFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('clientPage');
  if (storedPage) {
...
const changePage = (page) => {
  ...
  localStorage.setItem('clientPage', currentPage.value);
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    console.log('Права на добавление задач:', state.canAddClient);
    
    state.canViewClient = state.globalPermissionsList.includes('core.view_client') || !!state.objectPermissionsDict['core.view_client'];
    console.log('Права на просмотр задач:', state.canViewClient);
