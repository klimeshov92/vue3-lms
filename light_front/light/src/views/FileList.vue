<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewFile" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'FileDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddFile" class="list-menu button-group">

        <router-link
          v-if="state.canAddFile"
          :to="{ name: 'FileCreate' }"
          class="button"
        >
          Создать
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('fileFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('filePage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка файлов с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  try {
    ...
    console.log('Список файлов загружен:', response.data); 
...
const changePage = (page) => {
  ...
  localStorage.setItem('filePage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('filePage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddFile = state.globalPermissionsList.includes('files.add_file');
    console.log('Права на добавление файлов:', state.canAddFile);
    
    state.canViewFile = state.globalPermissionsList.includes('files.view_file') || !!state.objectPermissionsDict['files.view_file'];
