<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewPublicPlan" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'PublicPlanDetail', params: { id: item.id },  query: { tab: 'details' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddPublicPlan" class="list-menu button-group">

        <router-link
          v-if="state.canAddPublicPlan"
          :to="{ name: 'PublicPlanCreate' }"
          class="button"
        >
          Создать
        </router-link>
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('public_planFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('public_planPage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  ...
  localStorage.setItem('public_planFilters', JSON.stringify(filters))
...
const changePage = (page) => {
  ...
  localStorage.setItem('public_planPage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('public_planFilters');
  localStorage.removeItem('public_planPage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddPublicPlan = state.globalPermissionsList.includes('bpms.add_public_plan');
    console.log('Права на добавление планов:', state.canAddPublicPlan);
    
    state.canViewPublicPlan = state.globalPermissionsList.includes('bpms.view_public_plan') || !!state.objectPermissionsDict['bpms.view_public_plan'];
