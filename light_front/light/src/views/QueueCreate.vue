  ...
  <div v-if="loading">
    ...
    <div class="form-page">
      ...
      <form @submit.prevent="createObject" class="form" id="form">
        ...
        <div class="form-field">
          <label for="auto_assignment" class="form-label">Автоназначение исполнителя:</label>
...
const createObject = async () => {
  ...
  try {
    ...
    const jsonResponse = await axios.post(`${baseUrl}/queues/`, jsonData, {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    router.push({ name: 'QueueDetail', params: { id: jsonResponse.data.id }  });
  } catch (error) {
