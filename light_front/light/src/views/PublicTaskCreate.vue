...
const createObject = async () => {
  ...
  try {
    ...
    const jsonResponse = await axios.post(`${baseUrl}/public_tasks/`, jsonData, {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    if (avatarFile.value) {
      ...
      const avatarPatchUrl = `${baseUrl}/public_tasks/${jsonResponse.data.id}/`;
    ...
    router.push({ name: 'PublicTaskDetail', params: { id: jsonResponse.data.id }  });
  } catch (error) {
