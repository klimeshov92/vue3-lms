<template>
  <div v-if="loading">
    <div v-if="state.canViewClient" class="detail-page">
          ...
          <div class="detail-card-info">
            ...
            <div class="detail-card-text">
              <div v-if="state.object.object_type == 'account'" class="detail-card-text-elem">
                <span class="detail-card-text-label">Аккаунт:</span> {{ state.object.account ? state.object.account.str : 'Нет итога' }}
              </div>
            ...
            <div class="detail-menu button-group">
              ...
              <router-link 
                v-if="state.canEditClient" 
                :to="{ name: 'ClientEdit', params: { id: state.object.id } }"
                class="button"
              >
                Изменить
              </router-link>
              ...
              <button 
                v-if="state.canDeleteClient" 
                @click="openClientDeleteModal"
                class="button"
              >
                Удалить
              </button>
            ...
            <div v-if="showClientDeleteModal" class="modal-overlay">
              <div class="modal">
                <div class="modal-header">
                  <h2 class="modal-header-h2">Удаление {{ state.object.name || 'Безымянный клиента' }}</h2>
                </div>
                <div class="minibutton-group modal-menu">
                  <button @click="confirmClientDelete()" class="minibutton">Подтвердить</button>
                  <button @click="closeClientDeleteModal" class="minibutton">Отменить</button>
                </div>
              </div>
            </div>
          ...
          <div v-if="activeTab" class="tabs-content">
            ...
            <div v-if="activeTab === 'details'" class="detail-tab">
              <div v-if="state.object.object_type == 'account'" class="detail-tab-elem">
                <span class="detail-tab-label">Аккаунт:</span> {{ state.object.account ? state.object.account.str : 'Нет итога' }}
              </div>
            ...
            <div v-if="activeTab === 'accountsGroupObjectPermissions'" class="table-tab">
              ...
              <AccountsGroupObjectPermissions :state="state" :fetchObject="fetchObject" :contentTypeModel="'client'" />

            </div>

            <div v-if="activeTab === 'accountObjectPermissions'" class="table-tab">

              <AccountObjectPermissions :state="state" :fetchObject="fetchObject" :contentTypeModel="'client'" />
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddClient = state.globalPermissionsList.includes('core.add_client');
    console.log('Права на добавление клиент:', state.canAddClient);
    ...
    console.log('Права на просмотр клиента:', state.canViewClient);

    state.canEditClient = state.globalPermissionsList.includes('core.change_client') ||
      (Array.isArray(state.objectPermissionsDict['core.change_client']) &&
      state.objectPermissionsDict['core.change_client'].includes(Number(id)));
    console.log('Права на редактирование клиента:', state.canEditClient);

    state.canDeleteClient = state.globalPermissionsList.includes('core.delete_client') ||
      (Array.isArray(state.objectPermissionsDict['core.delete_client']) &&
      state.objectPermissionsDict['core.delete_client'].includes(Number(id)));
    console.log('Права на удаление клиента:', state.canDeleteClient);
...
const activeTab = ref(route.query.tab || localStorage.getItem(`activeClientTab-${route.params.id}`) || 'details');
watch(activeTab, (newTab) => {
  localStorage.setItem(`activeClientTab-${route.params.id}`, newTab);
