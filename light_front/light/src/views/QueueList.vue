<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewQueue" class="list-page">
        ...
        <div class="list-grid">
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'QueueDetail', params: { id: item.id },  query: { tab: 'desc' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddQueue" class="list-menu button-group">

        <router-link
          v-if="state.canAddQueue"
          :to="{ name: 'QueueCreate' }"
          class="button"
        >
          Создать
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('queueFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('queuePage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка очередей с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  localStorage.setItem('queueFilters', JSON.stringify(filters))
  ...
  try {
    ...
    console.log('Список очередей загружен:', response.data); 
    state.items = response.data.results;
    //state.items = [];
    totalPages.value = Math.ceil(response.data.count / 12);
  } catch (error) {
    console.error('Ошибка при загрузке списка очередей:', error);
...
const changePage = (page) => {
  ...
  localStorage.setItem('queuePage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('queueFilters');
  localStorage.removeItem('queuePage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddQueue = state.globalPermissionsList.includes('bpms.add_queue');
    console.log('Права на добавление очередей:', state.canAddQueue);
    
    state.canViewQueue = state.globalPermissionsList.includes('bpms.view_queue') || !!state.objectPermissionsDict['bpms.view_queue'];
