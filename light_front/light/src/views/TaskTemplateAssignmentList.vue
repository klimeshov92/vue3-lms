<template>
  ...
  <div v-if="loading">
    ...
    <div v-if="state.canViewTaskTemplateAssignment" class="list-page">

      <div class="list-header">
        <h1>Назначения шаблонов задач</h1>
      </div>
          ...
          <div v-for="item in state.items" :key="item.id" class="list-card">
            ...
            <div class="list-card-header">
              <div class="list-card-header-title">
                <h2>{{ item.plan ? item.plan.name + ' - ' : '' }}{{ item.name || 'Нет названия' }}</h2>
              </div>
            </div>
            ...
            <div class="list-card-menu minibutton-group">
              <router-link :to="{ name: 'TaskTemplateAssignmentDetail', params: { id: item.id },  query: { tab: 'desc' } }" class="minibutton">
                Подробнее
              </router-link>
            </div>
      ...
      <div v-if="state.canAddTaskTemplateAssignment" class="list-menu button-group">

        <router-link
          v-if="state.canAddTaskTemplateAssignment"
          :to="{ name: 'TaskTemplateAssignmentCreate' }"
          class="button"
        >
          Создать
        </router-link>
...
const state = reactive({
  ...
  canAddTaskTemplateAssignment: false,
...
const loadFilters = () => {
  const storedFilters = localStorage.getItem('task_template_assignmentFilters');
  if (storedFilters) {
...
const loadCurrentPage = () => {
  const storedPage = localStorage.getItem('task_template_assignmentPage');
  if (storedPage) {
...
const loadObjects = async (filters, page) => {
  console.log('Загрузка назначений шаблонов задач с фильтрами:', filters, 'для страницы:', page);
  let token = localStorage.getItem('access_token');
  const validToken = isTokenValid(token);
  if (token && !validToken) { 
    localStorage.removeItem('access_token');
  ...
  localStorage.setItem('task_template_assignmentFilters', JSON.stringify(filters))

  try {
    const response = await axios.get(`${baseUrl}/task_template_assignments/`, {
      headers: {
        ...(token && { 'Authorization': `Bearer ${token}` })
      },
      params: {
    ...
    console.log('Список назначений шаблонов задач загружен:', response.data); 
    state.items = response.data.results;
    //state.items = [];
    totalPages.value = Math.ceil(response.data.count / 12);
  } catch (error) {
    console.error('Ошибка при загрузке списка назначений шаблонов задач:', error);
...
const changePage = (page) => {
  ...
  localStorage.setItem('task_template_assignmentPage', currentPage.value);
...
const resetFilters = () => {
  ...
  localStorage.removeItem('task_template_assignmentFilters');
  localStorage.removeItem('task_template_assignmentPage');
  currentPage.value = 1;
...
const loadUserPermissions = async () => {
  ...
  try {
    ...
    state.canAddTaskTemplateAssignment = state.globalPermissionsList.includes('bpms.add_task_template_assignment');
    console.log('Права на добавление назначений шаблонов задач:', state.canAddTaskTemplateAssignment);
    
    state.canViewTaskTemplateAssignment = state.globalPermissionsList.includes('bpms.view_task_template_assignment') || !!state.objectPermissionsDict['bpms.view_task_template_assignment'];
    console.log('Права на просмотр назначений шаблонов задач:', state.canViewTaskTemplateAssignment);
