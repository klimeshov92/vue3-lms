          ...
          <div class="detail-card-info">
            <div class="detail-header"> 
              <div class="detail-header-title">
                <h1>{{ state.object.task.material.name || 'Безымянный материал' }}</h1>
              </div>
            </div>
            <div class="material-content">
              <div v-html="state.object.task.material.content" class="tiptap"></div>
            </div>
            <div class="detail-menu button-group">

              <button
                v-if="state.canCompleted"
                @click="materialReview"
                class="button"
              >
...
const fetchObject = async () => {
  ...
  try {
    const response = await axios.get(`${baseUrl}/material_content/${id}/`, {
      headers: {
...
const materialReview = async () => {
  ...
  try {
    const response = await axios.patch(`${baseUrl}/material_review/${id}/`, {}, {
      headers: {
        ...(token && { Authorization: `Bearer ${token}` })
      }
    });
