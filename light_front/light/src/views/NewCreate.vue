...
const createObject = async () => {
  ...
  try {
    ...
    const jsonResponse = await axios.post(`${baseUrl}/news/`, jsonData, {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    if (avatarFile.value) {
      ...
      const avatarPatchUrl = `${baseUrl}/news/${jsonResponse.data.id}/`;
    ...
    router.push({ name: 'NewDetail', params: { id: jsonResponse.data.id }  });
  } catch (error) {
